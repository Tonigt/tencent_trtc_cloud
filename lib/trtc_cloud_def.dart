...
class TRTCCloudDef {
  ...
  */
  ...
  /// 安卓视频渲染使用TextureView
  static final int TRTC_VideoView_TextureView = 1;

  /// 安卓视频渲染使用SurfaceView
  static final int TRTC_VideoView_SurfaceView = 2;
}
...
class TRTCParams {
  ...
  String userDefineRecordId; // 云端录制开关，用于指定是否要在云端将该用户的音视频流录制成指定格式的文件
  ...
  String
      privateMapKey; // 房间签名 [非必填]，当您希望某个房间只能让特定的 userId 进入时，需要使用 privateMapKey 进行权限保护。

  /// 业务数据 [非必填] 某些非常用的特殊需求才需要用到此字段。
  ///
  /// 【推荐取值】不建议使用
  String businessInfo; // 业务数据 [非必填] 某些非常用的特殊需求才需要用到此字段。
...
class TRTCTranscodingConfig {
  ...
  ///【字段含义】每一路子画面的位置信息
  List<TRTCMixUser>? mixUsers; // 每一路子画面的位置信息
